...
final class TestCollectionViewDriver: UnitTestCase {
    ...
    @MainActor
    func test_delegate_willDisplay_calls_cellViewModel() {
        let cell = FakeCollectionCell()
        let sections = 2
        let cells = 5
        let model = self.fakeCollectionViewModel(numSections: sections, numCells: cells, expectWillDisplay: true)
        let driver = CollectionViewDriver(
            view: self.collectionView,
            viewModel: model,
            options: .test()
    ...
    @MainActor
    func test_delegate_didEndDisplaying_calls_cellViewModel() {
        let cell = FakeCollectionCell()
        let sections = 2
        let cells = 5
        let model = self.fakeCollectionViewModel(numSections: sections, numCells: cells, expectDidEndDisplaying: true)
        let driver = CollectionViewDriver(
            view: self.collectionView,
            viewModel: model,
            options: .test()
    ...
    @MainActor
    func test_delegate_willDisplay_calls_supplementaryViewModel() {
        let cell = FakeCollectionCell()
    ...
    @MainActor
    func test_delegate_didEndDisplaying_calls_supplementaryViewModel() {
        let cell = FakeCollectionCell()
        let view = FakeSupplementaryView()
        let sections = 2
        let cells = 5
